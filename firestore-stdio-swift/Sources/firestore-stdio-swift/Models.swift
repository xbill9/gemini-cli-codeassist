...
struct FirestoreValue: Codable, Sendable {
    ...
    var integerValue: String? // Firestore returns integers as strings in JSON to avoid 64-bit precision loss
...
extension Product {
    func toFirestoreFields() -> [String: FirestoreValue] {
        return [
            ...
            "price": FirestoreValue(doubleValue: price), // Firestore might expect doubleValue or integerValue.
            ...
            "actualdateadded": FirestoreValue(timestampValue: ISO8601DateFormatter().string(from: actualdateadded))
        ]
    }
    
    static func from(document: FirestoreDocument) -> Product? {
        guard let fields = document.fields,
              let namePath = document.name else { return nil }
        ...
        let price: Double
        if let d = priceVal.doubleValue { price = d }
        else if let i = priceVal.integerValue, let d = Double(i) { price = d }
        else { return nil }
        
        // Handle quantity
        let quantity: Int
        if let i = quantityVal.integerValue, let q = Int(i) { quantity = q }
        else { return nil }
