...
This is a **Zig-based Model Context Protocol (MCP) server** named `firestore-https-zig`. It uses the `mcp` Zig library (managed via Zig Package Manager) to implement the protocol.
...
*   `build.zig`: Build configuration for compiling the Zig executable (`firestore-https-zig`) and linking dependencies.
