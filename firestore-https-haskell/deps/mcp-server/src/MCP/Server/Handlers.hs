...
logJson :: MonadIO m => Text -> Text -> m ()
logJson level msg = liftIO $ do
  now <- getCurrentTime
  let logEntry = object
        [ "timestamp" .= now
        , "level" .= level
...
handleMcpMessage _ _ (JsonRpcMessageNotification notif) = do
  case notificationMethod notif of
    "notifications/initialized" -> do
      logJson "INFO" "Received initialized notification - server is ready for operation"
      return ()
    _ -> do
      logJson "INFO" $ "Received unknown notification: " <> notificationMethod notif
      return ()
  return Nothing
        ...
        Success initReq -> do
          ...
          case validateProtocolVersion clientVersion of
            ...
            Right negotiatedVersion -> do
              logJson "INFO" $ "Client version: " <> clientVersion <> ", using: " <> negotiatedVersion
