...
logJson :: HttpConfig -> Text -> Text -> IO ()
logJson config level msg = when (httpVerbose config || level /= "DEBUG") $ do
  now <- getCurrentTime
  let logEntry = object
        [ "timestamp" .= now
        , "level" .= level
  ...
  BSLC.hPutStrLn stderr (encode logEntry)

-- | Helper for verbose logging
logVerbose :: HttpConfig -> String -> IO ()
logVerbose config msg = logJson config "DEBUG" (T.pack msg)
...
transportRunHttp config serverInfo handlers = do
  ...
  logJson config "INFO" $ T.pack $ "Starting MCP HTTP server on " ++ httpHost config ++ ":" ++ show (httpPort config) ++ httpEndpoint config
...
handleMcpRequest config serverInfo handlers req respond = do
  -- Check for MCP-Protocol-Version header (2025-06-18 requirement)
  -- We're lenient here for compatibility with various clients
  case lookup "MCP-Protocol-Version" (Wai.requestHeaders req) of
    Nothing -> 
      logVerbose config "Notice: Missing MCP-Protocol-Version header, assuming 2025-06-18"
    Just headerValue -> 
      if TE.decodeUtf8 headerValue /= "2025-06-18" then
        logVerbose config $ "Notice: Client requested protocol version " ++ show headerValue ++ ", but server only supports 2025-06-18"
      else
        return ()
...
handleJsonRpcRequest config serverInfo handlers body respond = do
  case eitherDecode body of
    Left err -> do
      logJson config "ERROR" $ T.pack $ "JSON parse error: " ++ err
      respond $ Wai.responseLBS
        status400
...
handleSingleJsonRpc config serverInfo handlers jsonValue respond = do
  case parseJsonRpcMessage jsonValue of
    Left err -> do
      logJson config "ERROR" $ T.pack $ "JSON-RPC parse error: " ++ err
      respond $ Wai.responseLBS
        status400
        [("Content-Type", "application/json")]
