...
has auth => sub { die "auth is required" }; # Expected to respond to get_project_id and get_access_token
...
sub batch_delete ($self, $ids) {
    ...
    my $project_id = $self->auth->get_project_id;
    my $url = "https://firestore.googleapis.com/v1/projects/$project_id/databases/(default)/documents:commit";
    
    my @writes;
    foreach my $id (@$ids) {
        push @writes, {
            delete => "projects/$project_id/databases/(default)/documents/inventory/$id"
        };
    }
...
sub _document_to_product ($self, $doc) {
    ...
    return undef unless defined $name && defined $price && defined $quantity && defined $imgfile;
