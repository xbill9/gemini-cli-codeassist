...
(defun handle-request (transport env)
  "Handle an incoming HTTP request. Monkey-patched to support root path '/""
...
(define-tool (user-tools greet)
  ...
  (mcp-server.logger:log-debug "Executed greet tool" (alexandria:plist-hash-table (list "param" param)))
...
(define-tool (user-tools get_products) ()
  ...
  (:result (serapeum:soft-list-of 40ants-mcp/content/text:text-content))
  (if (not mcp-server.firestore:*db-running*)
      (list (make-instance '40ants-mcp/content/text:text-content
                           :text "Inventory database is not running."))
      (handler-case
          (let ((products (mcp-server.firestore:get-products)))
            (list (make-instance '40ants-mcp/content/text:text-content
                                 :text (with-output-to-string (s) (yason:encode products s)))))
        (error (e)
          (mcp-server.logger:log-info (format nil "Error fetching products ~A" e))
          (list (make-instance '40ants-mcp/content/text:text-content
...
(define-tool (user-tools get_product_by_id) (id)
  ...
  (if (not mcp-server.firestore:*db-running*)
      (list (make-instance '40ants-mcp/content/text:text-content
                           :text "Inventory database is not running."))
      (handler-case
          (let ((product (mcp-server.firestore:get-product-by-id id)))
            (if product
                (list (make-instance '40ants-mcp/content/text:text-content
                                     :text (with-output-to-string (s) (yason:encode product s))))
                (list (make-instance '40ants-mcp/content/text:text-content
                                     :text "Product not found."))))
        (error (e)
          (mcp-server.logger:log-info (format nil "Error fetching product ~A ~A" id e))
          (list (make-instance '40ants-mcp/content/text:text-content
                               :text "Error fetching product.")))))

(define-tool (user-tools search) (query)
  (:summary "Search products")
  (:description "Search for products in the inventory database by name")
  (:param query string "The search query to filter products by name")
  (:result (serapeum:soft-list-of 40ants-mcp/content/text:text-content))
  (if (not mcp-server.firestore:*db-running*)
      (list (make-instance '40ants-mcp/content/text:text-content
                           :text "Inventory database is not running."))
      (handler-case
          (let ((results (mcp-server.firestore:search-products query)))
            (list (make-instance '40ants-mcp/content/text:text-content
                                 :text (with-output-to-string (s) (yason:encode results s)))))
        (error (e)
          (mcp-server.logger:log-info (format nil "Error searching products ~A" e))
          (list (make-instance '40ants-mcp/content/text:text-content
...
(define-tool (user-tools seed) ()
  ...
  (if (not mcp-server.firestore:*db-running*)
      ...
      (handler-case
          (progn
            (mcp-server.firestore:seed-database)
            (list (make-instance '40ants-mcp/content/text:text-content
                                 :text "Database seeded successfully.")))
        (error (e)
          (mcp-server.logger:log-info (format nil "Error seeding database ~A" e))
          (list (make-instance '40ants-mcp/content/text:text-content
                               :text (format nil "Error seeding database ~A" e))))))
...
(define-tool (user-tools reset) ()
  ...
  (if (not mcp-server.firestore:*db-running*)
      ...
      (handler-case
          (progn
            (mcp-server.firestore:reset-database)
            (list (make-instance '40ants-mcp/content/text:text-content
                                 :text "Database reset successfully.")))
        (error (e)
          (mcp-server.logger:log-info (format nil "Error resetting database ~A" e))
          (list (make-instance '40ants-mcp/content/text:text-content
                               :text (format nil "Error resetting database ~A" e))))))
...
(defun main (&rest argv)
  ...
  (setf *debugger-hook* (lambda (c h)
                          (declare (ignore h))
                          (mcp-server.logger:log-json "ERROR" "Unhandled condition"
                                    (alexandria:plist-hash-table
                                     (list "condition" (format nil "~A" c))))
                          (uiop:quit 1)))
  (handler-case
      (handler-bind ((warning (lambda (c)
                                (mcp-server.logger:log-json "WARN" (format nil "~A" c))
                                (muffle-warning c))))
        (let ((port (if (uiop:getenv "PORT")
                        (parse-integer (uiop:getenv "PORT"))
                        8080)))
          (mcp-server.logger:log-info "Initializing Firestore...")
          (mcp-server.firestore:init-firestore)
          (mcp-server.logger:log-info (format nil "Starting MCP server on port ~A..." port))
          (start-server user-tools :transport :http :port port)))
    (error (c)
      (mcp-server.logger:log-json "ERROR" "Fatal error in main loop"
                (alexandria:plist-hash-table
                 (list "error" (format nil "~A" c))))
      (uiop:quit 1))
    (sb-sys:interactive-interrupt ()
      (mcp-server.logger:log-info "Exiting via interrupt...")
      (uiop:quit 0))))